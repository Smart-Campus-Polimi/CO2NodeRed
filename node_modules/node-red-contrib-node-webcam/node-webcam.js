...
// default consts
    ...
    function WebcamTakePhotoNode(config) {
    // Create this node
...
// option value
            ...
            if (RED.settings.verbose) { node.log("webcam:"+filefqn); }
            ...
            (function () {
                ...
                Webcam.capture( filefqn, function( err, data ) {
                    if ( err ) {
                        ...
                        msg.payload = {error: "Webcam error: "+ err};
                        node.status({});
                        node.send(msg);
                        return;
                    }
                    ...
                    if ( filemode === "0" ) {
                        ...
                        msg.fileformat = "";
                        msg.filepath = "";
                        console.log("Webcam: written to buffer with success! retry=" + retry);
                    } else {
                        msg.filename = filename;
                        msg.filepath = filepath;
                        msg.fileformat = fileformat;
                        console.log("Webcam: written to " + data + " with success! retry=" + retry);
                    }
                    node.status({});
                    node.send(msg);
                    Webcam.clear();
    ...
    RED.nodes.registerType("webcam",WebcamTakePhotoNode);
}
